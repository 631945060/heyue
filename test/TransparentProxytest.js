...
describe("Transparent Proxy Upgrade Test", async function () {
    let deployer, user;
    let logicV1, logicV2, proxy,proxyDres;
    let proxyAsV1;
    beforeEach(async function () {
        ...
        proxyDres =await proxy.getAddress();
        console.log(" proxy.address:" + proxyDres);
        // 3. 将 proxy 转换为 V1 接口
        proxyAsV1 = await ethers.getContractAt("BeggingContractTransparentV1", proxyDres);
    ...
    it("Should upgrade to V2 and retain state", async function () {
        ...
        // // 4. 将 proxy 转换为 V2 接口
        const proxyAsV2 = await ethers.getContractAt("BeggingContractTransparentV2",proxyDres);
